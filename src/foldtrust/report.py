...
def generate_html_report(result: Dict, output_path: Path):
    ...
    if references:
        ...
        for ref in references:
            ...
            if doi:
                refs_html += f'<li><a href="https://doi.org/{doi}" target="_blank">{ref_text}</a></li>'
            elif pmid:
...
{f"- **Disease:** {disease}" if disease != "N/A" else ""}
{f"- **Gene:** {gene}" if gene != "N/A" else ""}

## Verdict

**{result['verdict']}**

{f"### Teaching Point\n\n{teaching_point}\n" if teaching_point else ""}
