...
def tier_distribution(
    ...
    for i, (t, c) in enumerate(zip(tier_order, counts)):
        if c > 0:
            ax.text(i, c + max(counts) * 0.02, str(int(c)), 
...
def shape_track(
    ...
    ax1.scatter(positions[valid_mask], shape_vals[valid_mask], 
               s=30, alpha=0.7, c='#A23B72', edgecolors='black', linewidth=0.5)
...
def disease_summary(
    ...
    ax.bar(x, floppy_counts, width, bottom=np.array(firm_counts) + np.array(soft_counts),
