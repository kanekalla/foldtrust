...
def classify_nucleotides_by_threshold(
    reactivity: np.ndarray, threshold_reactive: float = 0.75, threshold_unreactive: float = 0.25
...
def analyze_tier_reactivity_corrected(
    sequence: str, reactivity: np.ndarray, dataset_name: str, structure_type: str = "MEA"
) -> Dict:
    """
    ...
    for tier1, tier2 in comparisons:
        ...
        if len(vals1) >= 3 and len(vals2) >= 3:
            try:
                ...
                pairwise_tests[f"{tier1}_vs_{tier2}"] = {"statistic": np.nan, "pvalue": np.nan}
        else:
            pairwise_tests[f"{tier1}_vs_{tier2}"] = {"statistic": np.nan, "pvalue": np.nan}
...
def shape_directed_folding_corrected(
    sequence: str, reactivity: np.ndarray, dataset_name: str, is_icshape: bool = False
) -> Dict:
    """
...
def create_all_figures(
    ...
    axes[0].set_title(
        f"SARS-CoV-2 FSE (NC_045512.2:{fse_start}-{fse_end})", fontsize=12, fontweight="bold"
    )
    ...
    for i, dataset in enumerate(datasets):
        ...
        axes[i].hist(
            df_sub["spearman"].dropna(), bins=30, alpha=0.7, color="gray", edgecolor="black"
        )
        ...
        if dataset in fse_metrics:
            ...
            if not np.isnan(fse_rho):
                ...
                percentile = (
                    (valid < fse_rho).sum() / len(valid) * 100 if len(valid) > 0 else np.nan
                )
    ...
    for i, result in enumerate(shape_results_all):
        plot_arc_diagram(
            result["mea_structure_unconstrained"],
            f"{result['dataset']}: MEA unconstrained",
            axes[i, 0],
        )
        plot_arc_diagram(
            result["mea_structure_shape"], f"{result['dataset']}: MEA SHAPE-directed", axes[i, 1]
        )
...
def run_layer4_shape_analysis(
    ...
    for dataset_name, full_reactivity in datasets.items():
        ...
        for structure_type in ["MEA", "MFE"]:
            ...
            for tier in ["FIRM", "SOFT", "FLOPPY", "unpaired"]:
                ...
                if stats["n"] > 0:
                    print(
                        f"  {tier}: n={stats['n']}, mean={stats['mean']:.3f}, "
    ...
    for dataset_name, full_reactivity in datasets.items():
        ...
        print(
            f"  Tier changes (MEA): {shape_result['tier_changes']['tier_changes']} / "
    ...
    results["shape_directed_folding"] = [
        {
            ...
            if k
            not in [
                "mfe_structure_unconstrained",
                "mfe_structure_shape",
                "mea_structure_unconstrained",
                "mea_structure_shape",
            ]
        }
        for r in shape_results_all
    ...
    for dataset_name in dataset_names:
        ...
        if not np.isnan(fse_rho):
            ...
            percentile = (
                ((valid_rho < fse_rho).sum() / n_windows * 100) if n_windows > 0 else np.nan
            )
    ...
    for record in sensitivity_results["auroc_thresholds"]:
        print(
            f"  {record['threshold_reactive']}/{record['threshold_unreactive']}: "
...
def main():
    ...
    parser.add_argument(
        "--cache-dir", type=Path, default=Path("data/_cache"), help="Path to data cache"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("benchmarks/outputs/layer4_shape"),
