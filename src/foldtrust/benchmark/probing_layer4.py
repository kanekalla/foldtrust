...
def compute_unpaired_probabilities(prob_matrix: np.ndarray) -> np.ndarray:
    ...
    for i in range(n):
        ...
        unpaired[i] = max(0.0, min(1.0, 1.0 - paired_prob / 2.0))  # Divide by 2 since we counted twice
...
def plot_shape_correlation(
    ...
    ax.plot(positions, unpaired_probs, label="Unpaired probability",
            linewidth=2, alpha=0.8, color='#2E86AB')
    
    valid_pos = positions[valid_mask]
    ax.scatter(valid_pos, reactivities[valid_mask], label="SHAPE reactivity",
              s=40, alpha=0.7, color='#A23B72', edgecolors='black', linewidth=0.5)
    
    ax.set_xlabel("Position (nt)", fontsize=12, fontweight='bold')
    ax.set_ylabel("Value", fontsize=12, fontweight='bold')
    ax.set_title(f"{case_name} - {dataset_name}: Position-wise",
