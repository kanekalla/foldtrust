...
def run_disease_window_synthesis(
    ...
    for case_dir in case_dirs:
        ...
        print(f"  Pairs: {tier_counts['firm']}/{tier_counts['soft']}/{tier_counts['floppy']} (F/S/FL)")
...
def plot_disease_synthesis(df: pd.DataFrame, figures_dir: Path) -> None:
    ...
    # 2. Trust scores
    ax = axes[0, 1]
    colors = ['#2E7D32' if score > 0.7 else '#F57C00' if score > 0.5 else '#C62828'
              for score in df["trust_score"]]
    ax.barh(df["case"], df["trust_score"], color=colors, alpha=0.8, edgecolor='black', linewidth=1.5)
    ...
    ax.bar(x, df["floppy_count"], bottom=df["firm_count"] + df["soft_count"],
           label="FLOPPY", color='#C62828', alpha=0.9)
    ax.set_xlabel("Disease Window", fontsize=12, fontweight='bold')
    ...
    ax.pie(rec_values, labels=rec_labels, colors=colors, autopct='%1.0f%%',
