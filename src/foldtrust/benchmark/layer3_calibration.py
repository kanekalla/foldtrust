...
def filter_by_sample_ids(
    structures: List[Dict], dataset_name: str, sample_ids: Dict[str, Set[str]]
) -> List[Dict]:
...
def collect_candidate_pairs(
    structures: List[Dict], prob_floor: float = 1e-3, temperature: float = 37.0
                ...
                stem_tier_results.append(
                    {
                        ...
                        "pair_ppv": (
                            stats["pairs_tp"] / stats["pairs_total"]
            ...
            stem_tier_results.append(
                {
                    ...
                    "stems_total": 0,
                    "pair_ppv": (
                        lonely_pairs_tp / lonely_pairs_total if lonely_pairs_total > 0 else np.nan
                    ),
                }
            )
...
def run_layer3_calibration(
    ...
    if sample_size:
        ...
        print(
            f"  Loaded {len(archiveii)} ArchiveII, {len(bprna)} bpRNA, {len(rfam)} Rfam (matching Layer 2)"
        )
    ...
    calibration_df = pd.DataFrame(
        {"bin_mean": bin_means, "bin_accuracy": bin_accs, "bin_count": bin_counts}
    )
    ...
    for tier in ["FIRM", "SOFT", "FLOPPY"]:
        ...
        if len(tier_subset) > 0:
            ...
            pooled_ppv = (
                tier_subset["tp"].sum() / tier_subset["total"].sum()
                if tier_subset["total"].sum() > 0
                else 0.0
            )
    ...
    for tier in ["FIRM", "SOFT", "FLOPPY"]:
        ...
        if len(tier_subset) > 0:
            ...
            candidate_tier_summaries.append(
                {"tier": tier, "pooled_ppv": pooled_ppv, "n_pairs": int(tier_subset["total"].sum())}
            )
    ...
    for _, row in mfe_tier_summary_df.iterrows():
        print(
            f"  {row['tier']:7s}: Pooled PPV = {row['pooled_ppv']:.3f}, "
    ...
    for _, row in stem_tier_summary_df.iterrows():
        print(
            f"  {row['tier']:7s}: Pair PPV = {row['pooled_pair_ppv']:.3f}, "
