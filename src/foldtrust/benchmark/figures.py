...
def plot_reliability_diagram(
    ...
    for x, y, count in zip(bin_means, bin_accs, bin_counts):
        ax.annotate(
            f"{count}", (x, y), textcoords="offset points", xytext=(0, 10), ha="center", fontsize=8
        )
...
def plot_f1_by_family(df: pd.DataFrame, output_path: Path, method: str = "mfe"):
...
def plot_f1_vs_length(
    df: pd.DataFrame, output_path: Path, methods: list = ["mfe", "mea", "centroid"]
):
    ...
    for method in methods:
        ...
        for i in range(len(length_bins) - 1):
            mask = (method_df["length"] >= length_bins[i]) & (
                method_df["length"] < length_bins[i + 1]
            )
            if mask.sum() > 0:
...
def plot_method_comparison(df: pd.DataFrame, output_path: Path, dataset: Optional[str] = None):
...
def plot_tier_ppv(tier_summary_df: pd.DataFrame, output_path: Path):
    ...
    ax.bar(
        x,
        means,
        color=[OKABE_ITO["blue"], OKABE_ITO["orange"], OKABE_ITO["vermillion"]],
        alpha=0.7,
    )
