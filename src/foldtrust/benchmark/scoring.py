...
def run_scoring_tests(output_dir: Path) -> Dict:
    ...
    if test1_pass:
        ...
        print(f"  ✓ PASS: Sensitivity={metrics['sensitivity']:.3f}, PPV={metrics['ppv']:.3f}, F1={metrics['f1']:.3f}")
    else:
        print(f"  ✗ FAIL")
    ...
    test3_pass = (
        abs(metrics["sensitivity"] - 0.5) < 0.001 and
        abs(metrics["ppv"] - 0.5) < 0.001 and
        abs(metrics["f1"] - 0.5) < 0.001
    )
    ...
    if test3_pass:
        ...
        print(f"  ✗ FAIL: Got sens={metrics['sensitivity']:.3f}, ppv={metrics['ppv']:.3f}, f1={metrics['f1']:.3f}")
    ...
    actual = np.array([0] * 9 + [1] * 1 +  # 10% correct
                     [0] * 25 + [1] * 25 +  # 50% correct
                     [0] * 4 + [1] * 36)    # 90% correct
