...
def analyze_temperature_sweep(sequence: str, case_name: str, temperatures: List[int]) -> Dict:
    ...
    for temp in temperatures:
        ...
        if common_pairs:
            same_tier = sum(1 for pair in common_pairs if baseline_tiers[pair] == current_tiers[pair])
        ...
        if baseline_pairs or current_pairs:
            structure_jaccard = len(baseline_pairs & current_pairs) / len(baseline_pairs | current_pairs)
        else:
...
def generate_robustness_report(df: pd.DataFrame, summary: Dict, output_dir: Path, temperatures: List[int]) -> None:
