...
def save_sample_ids(archiveii: List[Dict], bprna: List[Dict], rfam: List[Dict], output_dir: Path):
    ...
    with open(sample_ids_path, "w", newline="") as f:
        ...
        for struct in archiveii:
            writer.writerow(
                ["ArchiveII", struct["name"], struct["length"], sequence_sha1(struct["sequence"])]
            )
        for struct in bprna:
            writer.writerow(
                ["bpRNA_TS0", struct["name"], struct["length"], sequence_sha1(struct["sequence"])]
            )
        for struct in rfam:
            writer.writerow(
                ["Rfam_seed", struct["name"], struct["length"], sequence_sha1(struct["sequence"])]
            )

    print(f"  Saved sample IDs to {sample_ids_path}")


def load_archiveii(cache_dir: Path, max_length: Optional[int] = 500) -> List[Dict]:
    """
...
def remove_pseudoknots(pairs: Set[Tuple[int, int]]) -> Tuple[Set[Tuple[int, int]], int]:
    ...
    for pair_idx, pair_elem in enumerate(pairs_list):
        ...
        if not isinstance(pair_elem, tuple) or len(pair_elem) != 2:
            raise ValueError(
                f"Expected (i, j) tuple at index {pair_idx}, got {pair_elem} (type={type(pair_elem)})"
            )
        a, b = pair_elem
...
def predict_structure_vienna(
    sequence: str, method: str = "mfe", temperature: float = 37.0, gamma: float = 1.0
) -> Set[Tuple[int, int]]:
    """
...
def bootstrap_ci(
    values: List[float], n_bootstrap: int = 1000, ci: float = 0.95, seed: int = 0
        ...
        for method in methods:
            try:
                pred_pairs = predict_structure_vienna(
                    sequence, method=method, temperature=temperature, gamma=gamma
                )
...
def compute_summary_stats(
    df: pd.DataFrame, group_cols: List[str], metric: str = "f1"
) -> pd.DataFrame:
...
def run_layer2_benchmark(
    ...
    results_df = evaluate_dataset(
        all_structures, methods=["mfe", "mea", "centroid"], temperature=37.0, gamma=1.0
    )
    ...
    summary_dataset = compute_summary_stats(results_df, ["dataset", "method"], metric="f1")
    ...
    if len(rfam_df) > 0:
        summary_family = compute_summary_stats(rfam_df, ["family", "method"], metric="f1")
        summary_family.to_csv(output_dir / "layer2_summary_family.csv", index=False)

    # Per length bin and method
    summary_length = compute_summary_stats(results_df, ["length_bin", "method"], metric="f1")
    ...
    summary = {
        ...
        "datasets": {"ArchiveII": len(archiveii), "bpRNA_TS0": len(bprna), "Rfam_seed": len(rfam)},
        "bootstrap_seed": 0,
...
if __name__ == "__main__":
    ...
    print()
    print(
        f"MFE F1: {summary['mfe_f1_mean']:.3f} "
        f"(95% CI: [{summary['mfe_f1_ci'][0]:.3f}, {summary['mfe_f1_ci'][1]:.3f}])"
    )
    print(
        f"MEA F1: {summary['mea_f1_mean']:.3f} "
