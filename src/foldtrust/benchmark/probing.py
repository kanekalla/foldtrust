...
from foldtrust.vienna import compute_pair_probabilities, fold_mfe
...
def run_probing_analysis(
    ...
    if len(reactivities) != len(sequence):
        print(f"  Error: Reactivity length ({len(reactivities)}) != sequence length ({len(sequence)})")
        return {}
...
def plot_reactivity_comparison(
    ...
    ax.set_title(f"{case_name}: Unpaired vs Reactivity\nSpearman ρ = {spearman:.3f} (p = {p_val:.2e})", fontsize=12, fontweight="bold")
    ax.grid(alpha=0.3)
    ...
    ax.plot(positions[valid_mask], reactivities[valid_mask], label="Reactivity", linewidth=1.5, alpha=0.8)
