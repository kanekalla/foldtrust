...
def render_layer3_tables(output_dir: Path) -> str:
    ...
    md.append(
        f"| Positive pairs | {summary['n_positive_pairs']} ({100*summary['n_positive_pairs']/summary['n_candidate_pairs']:.2f}%) |"
    )
    ...
    for i, (_, row) in enumerate(cal_curve.iterrows()):
        ...
        md.append(
            f"| {bin_label} | {row['bin_mean']:.3f} | {row['bin_accuracy']:.3f} | {int(row['bin_count'])} |"
        )
