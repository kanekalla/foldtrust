...
stems = re.findall(
    r"stem_loop\s+(\d+)\.\.(\d+)\s+/gene=\"ORF1ab\".*?/function=\"([^\"]+)\"", gb, re.S
)
...
out = {
    ...
    "genbank_stem_loops": [
        {"start": a, "end": b, "function": f, "sequence_RNA": g[a - 1 : b].replace("T", "U")}
        for a, b, f in stems
    ],
    "genbank_ORF1ab_CDS_ribosomal_slippage": cds.group(1) if cds else None,
    "fse_span_slippery_site_to_end_of_stem_loop_2": {
        "start": s,
        "end": fse_end,
        "length": fse_end - s + 1,
        "sequence_RNA": g[s - 1 : fse_end].replace("T", "U"),
    },
}
