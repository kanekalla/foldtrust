...
def parse_ct(text):
    ...
    for k, row in enumerate(rows, 1):
        f = row.split()
        assert int(f[0]) == k, f"bad index line {k}"
        seq.append(f[1])
        partner[k] = int(f[4])
...
def to_dotbracket(n, pairs):
    db = ["."] * n
    levels = [[] for _ in LEVELS]
    unplaced = 0
    for i, j in sorted(pairs):
        for L, lv in enumerate(levels):
            if all(not (a < i < b < j or i < a < j < b) for a, b in lv):
                lv.append((i, j))
                db[i - 1], db[j - 1] = LEVELS[L][0], LEVELS[L][1]
                break
        else:
            unplaced += 1
    return "".join(db), sum(len(level) for level in levels[1:]), unplaced
...
def main(tgz, out, url):
    ...
    with tarfile.open(tgz) as tf, open(out, "w") as fo:
        members = sorted(
            (m for m in tf.getmembers() if m.isfile() and m.name.endswith(".ct")),
            key=lambda m: m.name,
        )
        for m in members:
            ...
            for i in range(1, n + 1):
                ...
                if j:
                    assert (
                        1 <= j <= n and j != i and partner[j] == i
                    ), f"{m.name}: asymmetric pair {i}-{j}"
                    if i < j:
                        pairs.append([i, j])
        ...
        json.dumps(
            {
                ...
                "per_family": dict(sorted(stats.items())),
                "ct_files_with_more_than_one_structure": multi,
            },
            indent=1,
        )
    )
