...
def parse_header_coords(header: str) -> Optional[Dict[str, Any]]:
    ...
    return {"accession": accession, "start": start, "end": end, "strand": strand}


def verify_case(
    case_dir: Path, cache_dir: Path, from_cache: Optional[Path] = None
) -> tuple[bool, str]:
    """Verify one case.
    ...
    if not sequence_file.exists():
        return False, "Missing sequence.fa"

    if not meta_file.exists():
        return False, "Missing meta.yaml"
    ...
    if "provenance" not in meta:
        return False, "Missing provenance block in meta.yaml"
...
def main():
    ...
    parser.add_argument(
        "--from-cache", type=Path, help="Use cached fixtures instead of fetching from NCBI"
    )
    parser.add_argument(
        "--cache-dir",
        type=Path,
        default=Path("data/_cache/ncbi"),
